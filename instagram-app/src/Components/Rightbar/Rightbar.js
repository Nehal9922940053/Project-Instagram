import React from 'react'
import './Rightbar.css'
import Post from '../Post/Post';
import { PostExplore } from '../data';

const Rightbar = () => {
  return (
    <div className='MainRightBar'>
    <div className='submainrightbar'>

    <div style={{ flex:1.9 }}>
    {PostExplore.map((item)=>(
       <Post item={item}/>
    ))}
   
  
    </div>

    <div style={{ flex:2,marginLeft:135 }}>
    <div  style={{ marginRight:"50px" }}>
      <div style={{ display:'flex', alignItems:"center",marginLeft:20, marginTop: 30, cursor:"pointer"}}>
      <img src="https://images.unsplash.com/photo-1608155686393-8fdd966d784d?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D" style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
        <div style={{marginTop:10, padding:"14px" }}>
          <p style={{textAlign:"start",marginTop:-10, fontWeight:"600", fontSize:"14px" }}>Sumank9787</p>
          <p style={{marginTop:-2, textAlign:"start", fontSize:"15px", color:'#A8A8A8' }}>Suman Khadka</p>
        </div>
        <div  style={{ marginLeft:"100px", cursor:"pointer" }}>
          <p  style={{fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Switch</p>
        </div>
      </div>

    <div style={{display:"flex" }}>
    <div>
    <div style={{  display:'flex', marginTop:10 ,alignItems:"center"}}>
      <div style={{ marginLeft:6}}>
        <p style={{color:"#777777", marginLeft:"15px",fontSize:"14px", fontWeight:'600'  }}>Suggested for you</p>
      </div>
      <div style={{ display:'flex',marginLeft:155 }}>
      <p style={{color:"#000000", cursor:"pointer", fontSize:"13.5px", fontWeight:'600'  }}>See All</p>
      </div>
</div>
      <div style={{ display:"flex", alignItems:'center', marginLeft:20, marginTop:20 }}>
      <img src="data:image/webp;base64," style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
     <div>
     <p style={{marginLeft:10  }}>Elon Musk</p>
     <p  style={{marginTop:-3 , color:"#A8A8A8",marginLeft:10 }}>Follow you</p>
     </div>
      <div style={{marginLeft:"111px", cursor:"pointer"  }}>
      <p style={{marginLeft:"30px",fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Follow</p>
      </div>
      </div>

      <div style={{ display:"flex", alignItems:'center', marginLeft:20, marginTop:20 }}>
      <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/f/fc/Bill_Gates_-_2023_-_P062021-967902_%28cropped%29.jpg/330px-Bill_Gates_-_2023_-_P062021-967902_%28cropped%29.jpg" style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
     <div>
     <p style={{marginLeft:10  }}>Bill Gates</p>
     <p  style={{marginTop:-3 , color:"#A8A8A8",marginLeft:10 }}>Follow you</p>
     </div>
      <div style={{marginLeft:"111px", cursor:"pointer"  }}>
      <p style={{marginLeft:"30px",fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Follow</p>
      </div>
      </div>


      <div style={{ display:"flex", alignItems:'center', marginLeft:20, marginTop:20 }}>
      <img src="https://images.wsj.net/im-758635/?width=1000&size=0.8" style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
     <div>
     <p style={{marginLeft:10  }}>Sundar Pichai</p>
     <p  style={{marginTop:-3 , color:"#A8A8A8",marginLeft:10 }}>Follow you</p>
     </div>
      <div style={{marginLeft:"95px", cursor:"pointer"  }}>
      <p style={{marginLeft:"30px",fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Follow</p>
      </div>
      </div>


      <div style={{ display:"flex", alignItems:'center', marginLeft:20, marginTop:20 }}>
      <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Mark_Zuckerberg_F8_2019_Keynote_%2832830578717%29_%28cropped%29.jpg/800px-Mark_Zuckerberg_F8_2019_Keynote_%2832830578717%29_%28cropped%29.jpg" style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
     <div>
     <p style={{marginLeft:10  }}>Mark Zuckerberg</p>
     <p  style={{marginTop:-3 , color:"#A8A8A8",marginLeft:10 }}>Follow you</p>
     </div>
      <div style={{marginLeft:"70px", cursor:"pointer"  }}>
      <p style={{marginLeft:"30px",fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Follow</p>
      </div>
      </div>

      <div style={{ display:"flex", alignItems:'center', marginLeft:20, marginTop:20 }}>
      <img src="https://cdn.zeebiz.com/sites/default/files/2018/05/10/37733-kalyankrishnamurthy-twitter.jpg" style={{ width:'45px', height:"45px", borderRadius:"50%", objectFit:"cover"}} alt="" />
     <div>
     <p style={{marginLeft:10  }}>Kalyan Krishnamurthy</p>
     <p  style={{marginTop:-3 , color:"#A8A8A8",marginLeft:10 }}>Follow you</p>
     </div>
      <div style={{marginLeft:"70px", cursor:"pointer"  }}>
      <p style={{fontSize:"14px",color:'#0095F6', fontWeight:"600" }}>Follow</p>
      </div>
      </div> 
      </div>
      </div>
      <p style={{ textAlign:'Start', marginLeft:20, fontSize:12, color:'#A8A8A8', marginTop:20 }}>©️ 2023 INSTAGRAM FROM META</p>
    </div>
    </div>
    </div>
    </div>
  )
}

export default Rightbar